...
export default function Login() {
  ...
  // Sync session on mount if redirected from PMS
  useEffect(() => {
    ...
    if (action === 'login_sync' && token && encodedUser) {
      try {
        ...
        console.error('Failed to parse synchronized session credentials', e);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (lower.includes('email') || lower.includes('no account') || lower.includes('not found')) {
        setErrorField('email');
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-5 text-left">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary-500 transition-all text-sm text-gray-900 bg-white outline-none ${
                  errorField === 'email'
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary-500 transition-all text-sm text-gray-900 bg-white pr-12 outline-none ${
                    errorField === 'password'
                      ? 'border-red-400 ring-1 ring-red-300'
                      : 'border-gray-200'
                  }`}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 px-4 bg-gradient-to-r from-primary to-primary/80 hover:opacity-95 text-primary-foreground rounded-xl font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-primary/25 cursor-pointer"
            >
              {isLoading ? (
                <>
            ...
            <div className="text-center">
              <button
                ...
                className="text-sm text-primary font-bold hover:opacity-85 transition-colors cursor-pointer"
              >
                Forgot Password?
              </button>
            </div>
