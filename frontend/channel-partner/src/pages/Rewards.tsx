    ...
    if (isLoading) {
        return (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh' }}>
                <div style={{ color: 'var(--primary-gold)' }}>Loading rewards catalog...</div>
            </div>
        );
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div className="glass-pane" style={{ padding: '0.8rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <Zap size={20} color="var(--primary-gold)" />
                    <div>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)', textTransform: 'uppercase' }}>Available Balance</p>
                        <p style={{ fontWeight: 700, fontSize: '1.2rem' }} className="text-premium-gradient">{availablePoints.toLocaleString()} pts</p>
                    </div>
                </div>
            </div>
                            ...
                            <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        padding: '1rem',
                                        marginTop: 'auto',
                                        background: availablePoints >= reward.pointCost ? 'linear-gradient(135deg, var(--primary-gold) 0%, #ecd06f 100%)' : 'rgba(255,255,255,0.05)',
                                        color: availablePoints >= reward.pointCost ? 'var(--bg-dark)' : 'var(--text-dim)',
                                        ...
                                        gap: '0.5rem',
                                        opacity: availablePoints >= reward.pointCost ? 1 : 0.5,
