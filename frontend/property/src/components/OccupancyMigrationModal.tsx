...
export default function OccupancyMigrationModal({
    ...
    const {
        ...
        queryKey: ['occupancy-readiness', propertyId],
        queryFn: () => propertiesService.getOccupancyReadiness(propertyId),
    ...
    const {
        ...
        queryKey: ['occupancy-shadow', propertyId],
        queryFn: () => propertiesService.getOccupancyShadowValidation(propertyId),
        enabled: isOpen && activeTab === 'shadow' && !!propertyId,
    });

    // Activation Mutation
    const activateMutation = useMutation({
        mutationFn: () => propertiesService.activateV2Occupancy(propertyId),
        onSuccess: (data) => {
            toast.success(data.message || 'Property successfully activated to V2 Canonical Occupancy!');
            queryClient.invalidateQueries({ queryKey: ['occupancy-readiness', propertyId] });
    ...
    const deactivateMutation = useMutation({
        mutationFn: () => propertiesService.deactivateV2Occupancy(propertyId),
        onSuccess: (data) => {
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h2 className="text-lg font-bold text-gray-900 dark:text-white">
                                    Occupancy Renovation: V2 Migration & Readiness
                                </h2>
                                                    ...
                                                    <tr key={rt.roomTypeId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                                                        ...
                                                        <td className="p-3 text-right">
                                                            <button
                                                                onClick={() => {
                                                                    onClose();
                                                                    navigate(`/room-types/edit/${rt.roomTypeId}`);
                                                                }}
                            ...
                            <div className="p-4 bg-blue-50/70 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-900 text-xs text-blue-900 dark:text-blue-300 flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="mt-0.5 text-blue-800 dark:text-blue-400">
                                        This non-mutating simulation runs representative guest parties through both the legacy V1 engine and canonical V2 engine side-by-side. Use this to review pricing differences and availability shifts before flipping the property switch.
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl p-6 border border-emerald-200 dark:border-emerald-800 shadow-2xl space-y-4">
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                ...
                                <button
                                    ...
                                    className="px-5 py-2 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 shadow-md"
                                >
                        ...
                        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl p-6 border border-amber-200 dark:border-amber-800 shadow-2xl space-y-4">
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                ...
                                <button
                                    ...
                                    className="px-5 py-2 rounded-xl text-xs font-bold bg-amber-600 hover:bg-amber-700 text-white flex items-center gap-1.5 shadow-md"
                                >
